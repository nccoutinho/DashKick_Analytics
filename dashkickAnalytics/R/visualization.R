...
track_performance <- function(team_name) {
  tryCatch({
    ...
    # Handle the error, e.g., print an error message or return a default plot
    print(paste("Error: ", e$message))
    # You can also return a default plot or handle the error in another way
    # For example, you can create a default plot and return it
    return(ggplot() + geom_point(aes(x = 1, y = 1), size = 5, color = "red") +
             ggtitle("Error occurred during API call"))
  })
}
...
top_20 <- function(df) {
  tryCatch({
    ...
    message('An error occurred: This function should not be called directly\n', e$message)
    return (NULL)
  })
}
